...
const BookingsChatBot = () => {
  ...
  const createOrder = async (bookingId) => {
    try {
      ...
      const response = await axios.post(
        `https://www.api.blueaceindia.com/api/v1/create-order-from-chatbot/${bookingId}?type=order`,
        {},
        {
          headers: {
  ...
  const handleCreateOrderClick = (booking) => {
  ...
  const confirmBooking = async (bookingId) => {
    try {
      ...
      if (result.success) {
        ...
        setBookings((prev) =>
          prev.map((booking) => (booking._id === bookingId ? { ...booking, status: "confirmed" } : booking)),
        )
        setTimeout(() => setSuccess(""), 3000)
      } else {
  ...
  const filteredBookings = bookings.filter((booking) => {
    ...
    const matchesName = nameFilter === "" || booking.name?.toLowerCase().includes(nameFilter.toLowerCase())

    const matchesPhone = phoneFilter === "" || booking.phone?.includes(phoneFilter)

    const matchesStatus = statusFilter === "" || booking.status === statusFilter

    const bookingDate = new Date(booking.createdAt)
    const matchesDateFrom = dateFromFilter === "" || bookingDate >= new Date(dateFromFilter)
    const matchesDateTo = dateToFilter === "" || bookingDate <= new Date(dateToFilter + "T23:59:59")
  ...
  const getStatusClass = (status) => {
    switch (status) {
      ...
      case "confirmed":
        return "comp-status-confirmed"
      case "completed":
        return "comp-status-completed"
      case "cancelled":
        return "comp-status-cancelled"
      default:
  ...
  return (
    <div className="comp-container">
      <div className="comp-card">
        ...
        <div className="comp-body">
          ...
          <div className="comp-filters">
            ...
            <div className="comp-filters-grid">
              ...
              <div className="comp-form-group">
                ...
                <select className="comp-select" value={statusFilter} onChange={(e) => setStatusFilter(e.target.value)}>
              ...
              <div className="comp-form-group">
                <button className="comp-btn comp-btn-outline" onClick={clearFilters} title="Clear all filters">
                  Clear Filters
                </button>
              </div>
            </div>
                        ...
                        <div className="comp-actions-cell">
                          ...
                          {booking.status === "confirmed" && (
                            <>
                              ...
                              <button
                                className="comp-btn comp-btn-info comp-btn-sm"
                                onClick={() => handleCreateOrderClick(booking)}
            ...
            <div className="comp-pagination">
              ...
              <div className="comp-pagination-numbers">
                {getPageNumbers().map((pageNum, index) => (
                  <button
                    key={index}
                    className={`comp-pagination-number ${
          ...
          <div className="comp-actions">
            <button className="comp-btn comp-btn-primary" onClick={handleRefresh} disabled={loading}>
              🔄 Refresh Data
            </button>
          </div>
        </div>
        ...
        <div className="comp-modal-overlay" onClick={() => setShowDetailsModal(false)}>
          <div className="comp-modal" onClick={(e) => e.stopPropagation()}>
            <div className="comp-modal-header">
              ...
              <button className="comp-modal-close" onClick={() => setShowDetailsModal(false)}>
                ×
              </button>
            </div>
            ...
            <div className="comp-modal-footer">
              ...
              {selectedBooking.status === "confirmed" && (
                <button
                  className="comp-btn comp-btn-info"
                  onClick={() => {
                    setShowDetailsModal(false)
                    handleCreateOrderClick(selectedBooking)
                  }}
                  disabled={createOrderLoading}
                >
                  📦 Create Order
              ...
              <button className="comp-btn comp-btn-outline" onClick={() => setShowDetailsModal(false)}>
                Close
              </button>
            </div>
          </div>
        ...
        <div className="comp-modal-overlay" onClick={() => setShowCancelModal(false)}>
          <div className="comp-modal" onClick={(e) => e.stopPropagation()}>
            <div className="comp-modal-header">
              ...
              <button className="comp-modal-close" onClick={() => setShowCancelModal(false)}>
                ×
              </button>
            </div>
            <div className="comp-modal-body">
      ...
      {showCreateOrderModal && selectedBooking && (
        <div className="comp-modal-overlay" onClick={() => setShowCreateOrderModal(false)}>
          <div className="comp-modal" onClick={(e) => e.stopPropagation()}>
            <div className="comp-modal-header">
              <h2 className="comp-modal-title">Create Order</h2>
              <button className="comp-modal-close" onClick={() => setShowCreateOrderModal(false)}>
                ×
              </button>
            </div>
            <div className="comp-modal-body">
              <p>Are you sure you want to create an order for this booking?</p>
              <div className="comp-detail-item" style={{ marginBottom: "20px" }}>
                <div className="comp-detail-label">Booking ID</div>
                <div className="comp-detail-value">{selectedBooking.Booking_id}</div>
              </div>
              ...
              <div className="comp-detail-item" style={{ marginBottom: "20px" }}>
                <div className="comp-detail-label">Service</div>
                <div className="comp-detail-value">{selectedBooking.selectedService}</div>
              </div>
              <div className="comp-detail-item" style={{ marginBottom: "20px" }}>
                <div className="comp-detail-label">Service Date</div>
                <div className="comp-detail-value">{formatDate(selectedBooking.serviceDate)}</div>
            ...
            <div className="comp-modal-footer">
              <button
                className="comp-btn comp-btn-info"
                onClick={() => createOrder(selectedBooking._id)}
                disabled={createOrderLoading}
              >
