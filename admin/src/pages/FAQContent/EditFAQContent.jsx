...
function EditFAQContent() {
    ...
    useEffect(() => {
        const fetchData = async () => {
            try {
                const { data } = await axios.get(`https://www.api.blueaceindia.com/api/v1/get-single-faq-content/${id}`)
                const faq = data.data;
                setFormData({
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            await axios.put(`https://www.api.blueaceindia.com/api/v1/update-faq-content/${id}`, payload, {
                headers: {
